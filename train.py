...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            # imgn_train = add_watermark_noise(
            #     img_train, 40, True, random_img, alpha=opt.alpha)
            if opt.self_supervised == "True":
                imgn_train_2 = add_watermark_noise(
                    img_train, 40, True, random_img, alpha=opt.alpha)
            else:
            ...
            img_train, imgn_train = Variable(
                img_train.cuda()), Variable(imgn_train.cuda())
            imgn_train_2 = Variable(imgn_train_2.cuda())
            if opt.net == "FFDNet":
                noise_sigma = 0 / 255.
                noise_sigma = torch.FloatTensor(
                    np.array([noise_sigma for idx in range(img_train.shape[0])]))
        ...
        with torch.no_grad():
            ...
            writer.add_scalar("SSIM_val", ssim_val, epoch + 1)
            print("\n[epoch %d] PSNR_val: %.4f SSIM_val: %.4f" %
...
if __name__ == "__main__":
    ...
    if opt.preprocess:
        print("Im prepare")
        prepare_data(data_path=config['train_data_path'],
                     patch_size=256, stride=128, aug_times=1, mode='color')
    main()
